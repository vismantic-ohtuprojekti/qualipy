...
def test_largest_with_invalid_histogram():
    hist = np.array([])
    assert arrayNpEquals(largest(hist, 0.5), np.array([]))

def test_calculate_derivates_with_valid_histogram():
    hist = np.array([1, 2, 4, 2, 1])
    assert arrayNpEquals(calculate_derivates(hist), np.array([1.0, 2.0, -2.0, -1.0]))

def test_calculate_derivates_with_invalid_histogram():
    hist = np.array([])
    assert arrayNpEquals(calculate_derivates(hist), np.array([]))

def test_calculate_local_min_values_with_valid_histogram():
    hist = np.array([2.0, 1.0, 2.0, 4.0, 3.0, 2.0, 3.0])
    assert calculate_local_min_values(hist, 2) == [LocationData(1, 1.0), LocationData(5, 2.0)]
...
def test_calc_mean_with_invalid_histogram():
    hist = np.array([])
    hist_2 = np.array([1.0, -1.0])

    assert calc_mean(hist) == 0.0
    assert calc_mean(hist_2) == 0.0

def test_variance_with_valid_histogram():
    hist = np.array([1.0, 2.0, 3.0, 2.0, 3.0])
    mean = calc_mean(hist)

    assert np.absolute(calc_variance(hist, mean) - 2.686) < 0.01
...
def test_standard_deviation_with_valid_histogram():
    ...
    assert np.absolute(calc_standard_deviation(hist) - 1.639) < 0.01
...
def test_roughness_with_valid_histogram():
    hist = np.array([0.0, 0.5, 1.0, 0.5, 2.0, 0.5, 0.0])
...
def test_calculate_peak_value_with_valid_histogram():
    hist = np.array([0.0, 0.5, 1.0, 0.5, 2.0, 0.5, 0.0, 2.0])

    assert arrayNpEquals(calculate_peak_value(hist), np.array([-1.0]).astype(np.float32))

def test_calculate_peak_value_with_invalid_histogram():
    hist = np.array([])

    assert arrayNpEquals(calculate_peak_value(hist), np.array([]))
